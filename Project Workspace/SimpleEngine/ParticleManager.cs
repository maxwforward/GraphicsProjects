...
namespace CPI411.SimpleEngine
{
    public class ParticleManager
    {
        ...
        //#################################################################################################################################
        public void UpdateMedium(float ElapsedGameTime)
        {
            activeMax = 0;
            for (int i = 0; i < particleMax; i++)
            {
                particles[i].UpdateMedium(ElapsedGameTime);
                if (particles[i].IsActive())
                {
                    int ax = activeMax * 4; // ax: vertexBuffer index
                    Particle pp = particles[i];
        ...
        //#################################################################################################################################
        public void UpdateAdvanced(float ElapsedGameTime, float resil, float frict)
        {
            activeMax = 0;
            for (int i = 0; i < particleMax; i++)
            {
                particles[i].UpdateAdvanced(ElapsedGameTime, resil, frict);
                if (particles[i].IsActive())
                {
                    int ax = activeMax * 4; // ax: vertexBuffer index
                    Particle pp = particles[i];
